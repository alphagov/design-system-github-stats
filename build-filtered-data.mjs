...
export async function analyseRepo (repo) {
  ...
  try {
    ...
    // Get the repo tree
    result.repoTree = repoData.getRepoTree(result.latestCommitSHA)
    ...
    if (result.directDependencies.length === 0) {
      ...
      result.indirectDependencies = await repoData.getDependencies(packageObjects, true)
    } else {
      result.directDependencies = await repoData.disambiguateDependencies(result.directDependencies)
    }
  } catch (error) {
