...
describe('RepoData', () => {
  ...
  describe('getVersionFromLockfile', () => {
    ...
    // Skipped for now till I can work out what's wrong with yarnLock's mock
    it.skip('should get version from yarn.lock', async () => {
      const repoData = new RepoData(repoOwner, repoName, serviceOwners)
      const lockfileContent = {
        data: `
...
`
    ...
    it('should get version from yarn YAML', async () => {
      const repoData = new RepoData(repoOwner, repoName, serviceOwners)
      const lockfileContent = {
        data: `
"govuk-frontend@^3.11.0":
