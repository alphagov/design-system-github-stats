...
export class RepoData {
   ...
   */
  async getIndirectDependencies (packageObjects) {
    const results = []
    ...
    for (const packageObject of packageObjects) {
      try {
        const lockfileType = this.getLockfileType(packageObject.path)
   ...
   */
  async disambiguateDependencies (dependencies) {
    const results = []
    // We want to try the root directory in all cases
    ...
    for (const dependency of dependencies) {
      try {
        if (/^[~^*]/.test(dependency.frontendVersion)) {
          const lockfileType = this.getLockfileType(dependency.packagePath)
