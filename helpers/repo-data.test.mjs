...
describe('RepoData', () => {
  ...
  describe('getDirectDependencies', () => {
    it('should return direct dependencies of govuk-frontend', () => {
      ...
      expect(result).toEqual([
        ...
        { packagePath: 'src/package.json', frontendVersion: '3.10.0' }
      ])
    })
  ...
  describe('disambiguateDependencies', () => {
    it('should disambiguate dependencies with version ranges using package-lock.json', async () => {
      ...
      expect(result).toEqual([
        { packagePath: 'package.json', frontendVersion: '3.12.0' }
      ])
    })
  ...
  describe('getDirectDependencyFromLockfile', () => {
    it('should return the version of govuk-frontend from package-lock.json', async () => {
      const repoData = new RepoData(repoOwner, repoName)
      ...
      const result = await repoData.getDirectDependencyFromLockfile(lockfileObject, 'package-lock.json', 'package.json')
      expect(result).toEqual({
        frontendVersion: '3.11.0',
        packagePath: 'package.json'
      })
    ...
    it('should return the version of govuk-frontend from yarn.lock', async () => {
      const repoData = new RepoData(repoOwner, repoName)
      const lockfileObject = {
        'govuk-frontend@^3.11.0': {
          version: '3.11.0'
      ...
      const result = await repoData.getDirectDependencyFromLockfile(lockfileObject, 'yarn.lock', 'package.json')
      expect(result).toEqual({
        frontendVersion: '3.11.0',
        packagePath: 'package.json'
      })
    ...
    it('should return undefined if govuk-frontend is not found in package-lock.json', async () => {
      const repoData = new RepoData(repoOwner, repoName)
      const lockfileObject = {
        packages: {
    ...
    it('should return undefined if govuk-frontend is not found in yarn.lock', async () => {
      const repoData = new RepoData(repoOwner, repoName)
      const lockfileObject = {
        'other-package@^1.0.0': {
          version: '1.0.0'
